...
bool ASTComparer::isSameAST(const std::shared_ptr<ProgramNode>& firstAST, const std::shared_ptr<ProgramNode>& secondAST) {
...
bool ASTComparer::isSameProcedure(const std::shared_ptr<ProcedureNode>& firstProc, const std::shared_ptr<ProcedureNode>& secondProc) {
...
bool ASTComparer::isSameStatementList(const std::shared_ptr<StatementListNode>& firstStatementList,
    ...
    if (firstStatementListStatements.size() != secondStatementListStatements.size()) {
        return false;
    }
    ...
    for (int i = 0; i < firstStatementListStatements.size(); i++) {
        ...
        if (!func(firstStatementListStatements[i], secondStatementListStatements[i])) {
            return false;
        }
    }
...
bool ASTComparer::isSameAssign(const std::shared_ptr<AssignNode>& firstAssign, const std::shared_ptr<AssignNode>& secondAssign) {
...
bool ASTComparer::isSamePrint(const std::shared_ptr<PrintNode>& firstPrint, const std::shared_ptr<PrintNode>& secondPrint) {
...
bool ASTComparer::isSameWhile(const std::shared_ptr<WhileNode>& firstWhile, const std::shared_ptr<WhileNode>& secondWhile) {
...
bool
ASTComparer::isSameExpression(const std::shared_ptr<ExpressionNode>& firstExpr, const std::shared_ptr<ExpressionNode>& secondExpr) {
...
bool ASTComparer::isSameCondExpression(const std::shared_ptr<ConditionalExpressionNode>& firstExpr,
    ...
    if (auto firstUnary = std::dynamic_pointer_cast<UnaryConditionalExpressionNode>(firstExpr)) {
        if (auto secondUnary = std::dynamic_pointer_cast<UnaryConditionalExpressionNode>(secondExpr)) {
            return isSameCondExpression(firstUnary->getConditionalExpression(), secondUnary->getConditionalExpression());
        }
    }
    ...
    if (auto firstBinary = std::dynamic_pointer_cast<BinaryConditionalExpressionNode>(firstExpr)) {
        if (auto secondBinary = std::dynamic_pointer_cast<BinaryConditionalExpressionNode>(secondExpr)) {
            return firstBinary->getBinaryConditionalExpressionType() == secondBinary->getBinaryConditionalExpressionType()
                && isSameCondExpression(firstBinary->getLeftConditionalExpression(), secondBinary->getLeftConditionalExpression())
                && isSameCondExpression(firstBinary->getRightConditionalExpression(), secondBinary->getRightConditionalExpression());
        }
    }
