...
void ASTPrinterVisitor::visitProgramNode(ProgramNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                         std::shared_ptr<ASTNode> proc) const {
    std::cout << "main:program\n";
}

void ASTPrinterVisitor::visitProcedureNode(ProcedureNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                           std::shared_ptr<ASTNode> proc) const {
    std::cout << node->getProcedureName() + ":proc\n";
}

void ASTPrinterVisitor::visitStatementListNode(StatementListNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                               std::shared_ptr<ASTNode> proc) const {
    std::cout << ":stmtLst\n";
}

void ASTPrinterVisitor::visitAssignNode(AssignNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                        std::shared_ptr<ASTNode> proc) const {
    std::cout << ":assign\n";
}

void ASTPrinterVisitor::visitReadNode(ReadNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                      std::shared_ptr<ASTNode> proc) const {
    std::cout << ":read\n";
}

void ASTPrinterVisitor::visitPrintNode(PrintNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                       std::shared_ptr<ASTNode> proc) const {
    std::cout << ":print\n";
}

void ASTPrinterVisitor::visitVariableNode(VariableNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                          std::shared_ptr<ASTNode> proc) const {
    std::cout << node->getVarName() + ":var\n";
}

void ASTPrinterVisitor::visitConstantNode(ConstantNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                          std::shared_ptr<ASTNode> proc) const {
    std::cout << node->getValue() + ":const\n";
}

void ASTPrinterVisitor::visitCallNode(CallNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                      std::shared_ptr<ASTNode> proc) const {
    std::cout << node->getProcedureName() + ":call\n";
}

void ASTPrinterVisitor::visitIfNode(IfNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                    std::shared_ptr<ASTNode> proc) const {
    std::cout << ":if\n";
}

void ASTPrinterVisitor::visitWhileNode(WhileNode *node, std::vector<std::shared_ptr<ASTNode>> parents,
                                       std::shared_ptr<ASTNode> proc) const {
    std::cout << ":while\n";
}
