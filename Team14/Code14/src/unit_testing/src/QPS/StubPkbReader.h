...
public:
    ...
    getUsesStmtPair(StatementType type) const override;

    std::vector<std::vector<Entity>> getUsesProcPair() const override;

    std::vector<Entity> getUsesTypeIdent(StatementType type,
                                         Variable &var) const override;
    ...
    std::vector<Entity> getFollowsTypeStmt(StatementType type,
                                           Statement &statement) const override;

    std::vector<Entity>
    getFollowsStarTypeStmt(StatementType type,
                           Statement &statement) const override;

    std::vector<Entity>
    getFollowsTypeWildcard(StatementType type) const override;

    std::vector<Entity>
    getFollowsStarTypeWildcard(StatementType type) const override;

    std::vector<Entity> getFollowsStmtType(Statement &statement,
                                           StatementType type) const override;

    std::vector<Entity>
    getFollowsStarStmtType(Statement &statement,
                           StatementType type) const override;

    std::vector<Entity>
    getFollowsWildcardType(StatementType type) const override;

    std::vector<Entity>
    getFollowsStarWildcardType(StatementType type) const override;

    bool isFollows(Statement &statement1, Statement &statement2) const override;

    bool isFollowsStar(Statement &statement1,
                       Statement &statement2) const override;
    ...
    std::vector<std::vector<Entity>>
    getModifiesStmtPair(StatementType type) const override;

    std::vector<std::vector<Entity>> getModifiesProcPair() const override;

    std::vector<Entity> getModifiesTypeIdent(StatementType type,
                                             Variable &var) const override;
    ...
    // pattern a (_, "x")
    std::vector<Entity> getAssignStmtsByRhs(std::string &rhs,
    ...
    // pattern a (v, "x")
    std::vector<std::vector<Entity>>
    getAssignStmtsVarPairByRhs(std::string &rhs,
    ...
    // pattern a ("x", "x")
    std::vector<Entity>
    getAssignStmtsByLhsRhs(Variable &lhs, std::string &rhs,
                           bool hasRhsWildCard) const override;

    std::vector<std::vector<Entity>>
    getParentPair(StatementType formerType,
                  StatementType latterType) const override;
    ...
    std::vector<Entity> getParentTypeStmt(StatementType type,
                                          Statement &statement) const override;

    std::vector<Entity>
    getParentStarTypeStmt(StatementType type,
                          Statement &statement) const override;

    std::vector<Entity>
    getParentTypeWildcard(StatementType type) const override;

    std::vector<Entity>
    getParentStarTypeWildcard(StatementType type) const override;

    std::vector<Entity> getParentStmtType(Statement &statement,
                                          StatementType type) const override;

    std::vector<Entity>
    getParentStarStmtType(Statement &statement,
                          StatementType type) const override;

    std::vector<Entity>
    getParentWildcardType(StatementType type) const override;

    std::vector<Entity>
    getParentStarWildcardType(StatementType type) const override;

    bool isParent(Statement &statement1, Statement &statement2) const override;

    bool isParentStar(Statement &statement1,
                      Statement &statement2) const override;
    ...
    std::vector<std::vector<Entity>>
    getNextPair(StatementType formerType,
                StatementType latterType) const override;
    ...
    std::vector<Entity>
    getNextStarSameStmt(StatementType stmtType) const override;

    std::vector<Entity> getNextTypeStmt(StatementType type,
                                        Statement &statement) const override;

    std::vector<Entity>
    getNextStarTypeStmt(StatementType type,
                        Statement &statement) const override;

    std::vector<Entity> getNextTypeWildcard(StatementType type) const override;

    std::vector<Entity>
    getNextStarTypeWildcard(StatementType type) const override;

    std::vector<Entity> getNextStmtType(Statement &statement,
                                        StatementType type) const override;

    std::vector<Entity> getNextStarStmtType(Statement &statement,
                                            StatementType type) const override;

    std::vector<Entity> getNextWildcardType(StatementType type) const override;

    std::vector<Entity>
    getNextStarWildcardType(StatementType type) const override;

    bool isNext(Statement &statement1, Statement &statement2) const override;

    bool isNextStar(Statement &statement1,
                    Statement &statement2) const override;
