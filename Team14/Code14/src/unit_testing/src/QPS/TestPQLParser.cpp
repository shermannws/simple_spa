...
TEST_CASE("processDeclarations Errors") {
    SECTION("SyntaxExceptions") {
        ...
        testcases.emplace_back("assignment a; Select a",
                               "Expected a declaration but found none");
        testcases.emplace_back("assign a Select s",
                               "Expected ; but found 'Select'");
        testcases.emplace_back("assign a a1; Select a1",
                               "Expected ; but found 'a1'");
        testcases.emplace_back("assign a;",
                               "Expected Select clause but found ''");
    ...
    SECTION("SemanticExceptions") {
        std::vector<std::pair<std::string, std::string>> testcases;
        testcases.emplace_back("Select s ",
                               "Undeclared synonym in Select clause");
        testcases.emplace_back("stmt s; assign s; Select s ",
                               "Trying to redeclare a synonym");
...
TEST_CASE("processSelect Errors") {
    SECTION("SyntaxException") {
        std::vector<std::pair<std::string, std::string>> testcases;
        testcases.emplace_back("stmt s; where s",
                               "Expected Select clause but found 'where'");
    ...
    SECTION("SemanticException") {
        ...
        testcases.emplace_back("assign a; Select s",
                               "Undeclared synonym in Select clause");
        testcases.emplace_back("assign Synonym; Select synonym",
                               "Undeclared synonym in Select clause");
...
TEST_CASE("processSuchThatClause Calls") {
    SECTION("Valid Calls(_, _)") {
        ...
        auto expectedClause = QPSTestUtil::createSuchThatClause(
                ClauseType::Calls, RefType::EntRef, RootType::Wildcard,
    ...
    SECTION("Valid Calls(p, procName)") {
        ...
        Query query = parser.parse();
        auto expectedClause = QPSTestUtil::createSuchThatClause(
                ClauseType::Calls, RefType::EntRef, RootType::Synonym,
                QueryEntityType::Procedure, "procedure", RefType::EntRef,
                RootType::Ident, QueryEntityType::Invalid, "procName");
        REQUIRE(*query.getSuchThat()[0] == *expectedClause);
    }
    ...
    SECTION("Valid Calls(procName, _)") {
        ...
        Query query = parser.parse();
        auto expectedClause = QPSTestUtil::createSuchThatClause(
                ClauseType::Calls, RefType::EntRef, RootType::Ident,
    ...
    SECTION("Valid Calls(procName, p)") {
        PQLParser parser("procedure procedure, q; Select procedure such that "
                         "Calls (\"procedure\", q)");
        Query query = parser.parse();
        auto expectedClause = QPSTestUtil::createSuchThatClause(
                ClauseType::Calls, RefType::EntRef, RootType::Ident,
...
TEST_CASE("processSuchThatClause Calls*") {
    SECTION("Valid Calls*(_, p)") {
        PQLParser parser(
                "print s1; procedure q; Select s1 such that Calls* (_,q)");
        Query query = parser.parse();
        auto expectedClause = QPSTestUtil::createSuchThatClause(
                ClauseType::CallsStar, RefType::EntRef, RootType::Wildcard,
                QueryEntityType::Invalid, "_", RefType::EntRef,
                RootType::Synonym, QueryEntityType::Procedure, "q");
        REQUIRE(*query.getSuchThat()[0] == *expectedClause);
    }
    ...
    SECTION("Valid Calls*(_, procName)") {
        ...
        auto expectedClause = QPSTestUtil::createSuchThatClause(
                ClauseType::CallsStar, RefType::EntRef, RootType::Wildcard,
                QueryEntityType::Invalid, "_", RefType::EntRef, RootType::Ident,
                QueryEntityType::Invalid, "procName");
        REQUIRE(*query.getSuchThat()[0] == *expectedClause);
    }

    SECTION("Valid Calls*(procName, procName)") {
        PQLParser parser(
                R"(procedure procedure; Select procedure such that Calls ("procedure", "procedure"))");
        Query query = parser.parse();
        auto expectedClause = QPSTestUtil::createSuchThatClause(
                ClauseType::Calls, RefType::EntRef, RootType::Ident,
                QueryEntityType::Invalid, "procedure", RefType::EntRef,
...
TEST_CASE("Invalid processSuchThat cases") {
    ...
    SECTION("Invalid Calls/Calls* queries") {
        ...
        parser = PQLParser("assign a; variable v; procedure p;Select a such "
                           "that Calls*(p, 1)");
        REQUIRE_THROWS_AS(parser.parse(), SyntaxException);

        parser = PQLParser(
                "assign a; variable v; Select a such that Calls(v,_)");
        REQUIRE_THROWS_AS(parser.parse(), SemanticException);
...
TEST_CASE("invalid multi-clause queries") {
    SECTION("Syntax Errors") {
        ...
        PQLParser parser("assign a; variable v; Select v such that "
                         "Modifies(a,v) and pattern a(_,_) ");
        REQUIRE_THROWS_AS(parser.parse(), SyntaxException);

        // Use both `and` & clause keyword (such that)
        parser = PQLParser("assign a; variable v; Select v such that "
                           "Modifies(a,v) and such that Follows(1, 2)");
        REQUIRE_THROWS_AS(parser.parse(), SyntaxException);

        // Use both `and` & clause keyword (pattern)
        parser = PQLParser("assign a; variable v; Select a pattern a(_,_) and "
                           "pattern a(v,_)");
        REQUIRE_THROWS_AS(parser.parse(), SyntaxException);

        // Trailing `and`
        parser = PQLParser(
                "assign a; variable v; Select a pattern v(\"y\",_) and ");
        REQUIRE_THROWS_AS(parser.parse(), SyntaxException);
    }
}
...
TEST_CASE("valid multi-clause queries") {
    ...
    SECTION("use AND in such that") {
        PQLParser parser("assign a; variable v; Select v such that "
                         "Modifies(a,v) and Follows*(1,2) and Uses(a,v)");
        Query query = parser.parse();
        ...
        auto c1 = QPSTestUtil::createSuchThatClause(
                ClauseType::Modifies, RefType::StmtRef, RootType::Synonym,
                QueryEntityType::Assign, "a", RefType::EntRef,
                RootType::Synonym, QueryEntityType::Variable, "v");
        auto c2 = QPSTestUtil::createSuchThatClause(
                ClauseType::FollowsStar, RefType::StmtRef, RootType::Integer,
                QueryEntityType::Invalid, "1", RefType::StmtRef,
                RootType::Integer, QueryEntityType::Invalid, "2");
        auto c3 = QPSTestUtil::createSuchThatClause(
                ClauseType::Uses, RefType::StmtRef, RootType::Synonym,
                QueryEntityType::Assign, "a", RefType::EntRef,
                RootType::Synonym, QueryEntityType::Variable, "v");
    ...
    SECTION("use AND in pattern") {
        ...
        auto c1 = QPSTestUtil::createPatternClause(
                ClauseType::Assign, "a", RootType::Wildcard, "_",
                ExpressionSpecType::Wildcard, "");
        auto c2 = QPSTestUtil::createPatternClause(
                ClauseType::Assign, "pattern", RootType::Synonym, "v",
                ExpressionSpecType::Wildcard, "");
        auto c3 = QPSTestUtil::createPatternClause(
                ClauseType::Assign, "a1", RootType::Ident, "y",
    ...
    SECTION("use AND with all clause types") {
        ...
        auto pc2 = QPSTestUtil::createPatternClause(
                ClauseType::Assign, "and", RootType::Synonym, "v",
                ExpressionSpecType::PartialMatch, "((1)+(x))");
        auto pc3 = QPSTestUtil::createPatternClause(
                ClauseType::Assign, "such", RootType::Synonym, "v",
                ExpressionSpecType::Wildcard, "");
        auto sc1 = QPSTestUtil::createSuchThatClause(
                ClauseType::Follows, RefType::StmtRef, RootType::Integer,
                QueryEntityType::Invalid, "1", RefType::StmtRef,
                RootType::Integer, QueryEntityType::Invalid, "2");
        auto sc2 = QPSTestUtil::createSuchThatClause(
                ClauseType::ParentStar, RefType::StmtRef, RootType::Integer,
                QueryEntityType::Invalid, "1", RefType::StmtRef,
                RootType::Integer, QueryEntityType::Invalid, "10");
