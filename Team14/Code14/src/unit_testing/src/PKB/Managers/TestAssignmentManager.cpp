...
TEST_CASE("Test Assignment Retrieval") {
    ...
    shared_ptr<Assignment> assignment1 = make_shared<Assignment>(
            Assignment(statement1, variableA, expression1));
    shared_ptr<Assignment> assignment2 = make_shared<Assignment>(
            Assignment(statement2, variableA, expression2));
    assignmentManager.storeAssignPattern(assignment1);
    assignmentManager.storeAssignPattern(assignment2);

    string empty;
    string query = "(x+y)";
    vector<Entity> assignLhsRhsWc = assignmentManager.getAssignStmtsByLhsRhs(
            *variableA, query, true);// pattern ("a", "_(x+y)_")
    REQUIRE(assignLhsRhsWc.size() == 1);
    REQUIRE(assignLhsRhsWc.at(0) == *statement1);
    vector<Entity> assignLhsRhsNoWc = assignmentManager.getAssignStmtsByLhsRhs(
            *variableA, query, false);// pattern ("a", "(x+y)")
    REQUIRE(assignLhsRhsNoWc.empty());
    REQUIRE(assignmentManager.getAssignStmtsByLhs(*variableA).size() ==
            2);// pattern ("a", "_")
    vector<Entity> assignRhsWc = assignmentManager.getAssignStmtsByRhs(
            query, true);// pattern ("_", "_(x+y)_")
    REQUIRE(assignRhsWc.size() == 1);
    REQUIRE(assignRhsWc.at(0) == *statement1);
    REQUIRE(assignmentManager.getAssignStmtsByRhs(query, false)
                    .empty());// pattern ("_", "(x+y)")
    REQUIRE(assignmentManager.getAllAssignStmts().size() ==
            2);// pattern ("_", "_")
}