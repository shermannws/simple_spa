...
TEST_CASE("Test While Pattern Storage") {
    ...
    REQUIRE_NOTHROW(whilePatternManager.storeWhilePattern(make_shared<Statement>(whileStmt), make_shared<vector<shared_ptr<Variable>>>(vector<shared_ptr<Variable>>{make_shared<Variable>(var)})));
}
