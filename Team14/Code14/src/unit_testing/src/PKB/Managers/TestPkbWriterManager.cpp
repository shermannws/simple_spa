...
TEST_CASE("Test PkbWriterManager - Triggering Stmt to Var Transitive "
          "Calculation through triggerTransitiveCalc") {

    SECTION("Test Modifes(s,v) and Uses - v1") {
        auto assignmentManager =
                std::make_shared<AssignPatternManager>(AssignPatternManager());
        auto entitiesManager =
                std::make_shared<EntitiesManager>(EntitiesManager());
        auto followsRelationshipManager =
                std::make_shared<FollowsRelationshipManager>();
        auto usesRelationshipManager =
                std::make_shared<UsesRelationshipManager>();
        ...
        auto nextRelationshipManager =
                std::make_shared<NextRelationshipManager>();
        ...
        modifiesProcRelationshipManager->storeRelationship(
                std::make_shared<Procedure>(procedureA),
                std::make_shared<Variable>(variableC));
        ...
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement1, variableA) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement1, variableB) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement1, variableC) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement1, variableX) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement1, variableY) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement1, variableZ) == false);

        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement2, variableA) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement2, variableB) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement2, variableC) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement2, variableX) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement2, variableY) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement2, variableZ) == false);

        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement3, variableA) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement3, variableB) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement3, variableC) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement3, variableX) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement3, variableY) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement3, variableZ) == false);

        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement4, variableA) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement4, variableB) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement4, variableC) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement4, variableX) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement4, variableY) == false);
        REQUIRE(modifiesRelationshipManager->isRelationship(
                        statement4, variableZ) == false);

        // BEFORE STATE - Uses
        REQUIRE(usesRelationshipManager->isRelationship(statement1,
                                                        variableA) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement1,
                                                        variableB) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement1,
        ...
        REQUIRE(usesRelationshipManager->isRelationship(statement2,
                                                        variableA) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement2,
                                                        variableB) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement2,
        ...
        REQUIRE(usesRelationshipManager->isRelationship(statement3,
                                                        variableA) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement3,
                                                        variableB) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement3,
        ...
        REQUIRE(usesRelationshipManager->isRelationship(statement4,
                                                        variableA) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement4,
                                                        variableB) == false);
        REQUIRE(usesRelationshipManager->isRelationship(statement4,
        ...
        REQUIRE(modifiesRelationshipManager->isRelationship(statement2,
                                                            variableA) == true);
        REQUIRE(modifiesRelationshipManager->isRelationship(statement2,
                                                            variableB) == true);
        REQUIRE(modifiesRelationshipManager->isRelationship(statement2,
