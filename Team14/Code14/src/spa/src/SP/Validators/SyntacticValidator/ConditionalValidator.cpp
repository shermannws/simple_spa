...
ConditionalValidator::ConditionalValidator(const std::vector<SPToken> &tokens)
    : tokens(tokens) {}
...
void ConditionalValidator::validate() {
    ...
    if (tokensDeque.front().getType() != TokenType::OpenRoundParenthesis) {
        throw SyntaxError("Syntax error: Invalid Condition - Wrap Conditional "
                          "Expression in \"( )\" ");
    }
...
void ConditionalValidator::validateConditionalExpression(
        std::deque<SPToken> &expression) {
    SPToken token = expression.front();

    if (token.getType() == TokenType::OpenRoundParenthesis) {
        // Find the matching closing parenthesis
        size_t closingParenthesisIndex =
                findMatchingClosingParenthesis(expression);
        ...
        std::deque<SPToken> subExpression(expression.begin() + 1,
                                          expression.begin() +
                                                  closingParenthesisIndex);

        // Check for invalid ((A > B)) cases
        if (subExpression.front().getType() ==
            TokenType::OpenRoundParenthesis) {
            size_t nextClosingParenthesisIndex =
                    findMatchingClosingParenthesis(subExpression);
            if (closingParenthesisIndex == nextClosingParenthesisIndex + 2) {
                throw SyntaxError("Syntax error: Invalid nesting in "
        ...
        // Option 2 - check next token after the initial ')', split left and
        // right
        if (closingParenthesisIndex < expression.size() - 1) {
            SPToken nextToken = expression[closingParenthesisIndex + 1];
            std::deque<SPToken> rightSubExpression(
                    expression.begin() + closingParenthesisIndex + 2,
                    expression.end());

            // Option 2.1 Conditional expr
            if (nextToken.getValue() == AppConstants::STRING_AND ||
                nextToken.getValue() == AppConstants::STRING_OR) {
                validateConditionalExpression(subExpression);
    ...
    } else if (token.getType() == TokenType::ConditionalOperator &&
               token.getValue() == AppConstants::STRING_NOT) {
        std::deque<SPToken> subExpression(expression.begin() + 1,
                                          expression.end());
        validateConditionalExpression(subExpression);
...
void ConditionalValidator::validateRelationalExpression(
        std::deque<SPToken> &expression) {
    std::deque<SPToken> LHS, RHS;
    std::string operatorAsString;
    std::tie(LHS, RHS, operatorAsString) =
            splitExpression(expression, TokenType::RelationalOperator);
...
void ConditionalValidator::validateExpr(std::deque<SPToken> &tokens) {
    ...
    if (!tokens.empty() &&
        tokens.front().getType() != TokenType::ArithmeticOperator) {
        throw SyntaxError("Syntax error: Expected arithmetic operator while "
                          "checking relational expr");
    }
...
void ConditionalValidator::validateTerm(std::deque<SPToken> &tokens) {
    ...
    if (currToken.getType() == TokenType::Name) {
        ...
        std::deque<SPToken> exprTokens(
                tokens.begin() + 1, tokens.begin() + closingParenthesisIndex);

        validateExpr(exprTokens);

        tokens.erase(tokens.begin(),
                     tokens.begin() + closingParenthesisIndex + 1);
    } else {
...
size_t ConditionalValidator::findMatchingClosingParenthesis(
        std::deque<SPToken> &tokens) {
    int nestingLvl = 1;
...
ConditionalValidator::splitExpression(std::deque<SPToken> &expression,
                                      TokenType type) {
