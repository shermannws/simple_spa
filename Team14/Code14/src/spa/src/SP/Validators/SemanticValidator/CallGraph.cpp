...
void CallGraph::addEdge(const std::string &caller, const std::string &callee) { edges[caller].push_back(callee); }
...
bool CallGraph::hasCycleDFS(const std::string &currentProcedure, std::unordered_set<std::string> &visited,
                            std::unordered_set<std::string> &currentPath) {
