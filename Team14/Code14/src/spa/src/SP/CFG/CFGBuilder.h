...
private:
    /**
     * Builds a CFG for the procedure represented by the subtree rooted at the
     * ProcedureNode given.
     * @param procedureNode The root (ASTNode) of the procedure subtree to
     * create a CFG for
     * @return Unordered map of Statement to CFGNode
     */
    static std::unordered_map<Statement, std::shared_ptr<CFGNode>>
    buildCFGForProcedure(const std::shared_ptr<ProcedureNode> &procedureNode);

    /**
     * Builds a CFG subgraph for a statement list.
     * @param map Unordered map of each procedure to a map of Statement to
     * CFGNode
     * @param statementListNode The root (ASTNode) of the statement list subtree
     * to create a CFG subtree for
     * @return The head and tail nodes of this subgraph
     */
    static std::pair<std::shared_ptr<CFGNode>, std::shared_ptr<CFGNode>>
    buildStatementListSubgraph(
            std::unordered_map<Statement, std::shared_ptr<CFGNode>> &map,
            const std::shared_ptr<StatementListNode> &statementListNode);

    /**
     * Builds a CFG subgraph for a statement (assign, call, print, read, if,
     * while).
     * @param map Unordered map of each procedure to a map of Statement to
     * CFGNode
     * @param statementNode The root (ASTNode) of the statement subtree to
     * create a CFG subtree for
     * @return The head and tail nodes of this subgraph
     */
    static std::pair<std::shared_ptr<CFGNode>, std::shared_ptr<CFGNode>>
    buildStatementSubgraph(
            std::unordered_map<Statement, std::shared_ptr<CFGNode>> &map,
     ...
     * @param map Unordered map of each procedure to a map of Statement to
     * CFGNode
     * @param ifNode The root (ASTNode) of the if statement subtree to create a
     * CFG subtree for
     * @return The head and tail nodes of this subgraph
     */
    static std::pair<std::shared_ptr<CFGNode>, std::shared_ptr<CFGNode>>
     ...
     * @param map Unordered map of each procedure to a map of Statement to
     * CFGNode
     * @param whileNode The root (ASTNode) of the while statement subtree to
     * create a CFG subtree for
     * @return The head and tail nodes of this subgraph
     */
    static std::pair<std::shared_ptr<CFGNode>, std::shared_ptr<CFGNode>>
...
public:
    /**
     * Builds a CFG for each procedure in the AST given. For each procedure,
     * returns an unordered map of Statement to CFGNode for random access using
     * Statements.
