...
std::shared_ptr<StatementListNode> IfNode::getThenStatementList() { return thenStatementList; }

std::shared_ptr<StatementListNode> IfNode::getElseStatementList() { return elseStatementList; }

std::vector<std::shared_ptr<ASTNode>> IfNode::getAllChildNodes() {
    std::vector<std::shared_ptr<ASTNode>> children{conditionalExpression, thenStatementList, elseStatementList};
    return children;
}
...
void IfNode::accept(std::shared_ptr<DesignExtractorVisitor> visitor, std::vector<std::shared_ptr<Statement>> parents,
    ...
    if (auto ifVisitor = std::dynamic_pointer_cast<IfNodeVisitor>(visitor)) {
        ifVisitor->visitIfNode(std::enable_shared_from_this<IfNode>::shared_from_this(), parents, proc);
    }
}
