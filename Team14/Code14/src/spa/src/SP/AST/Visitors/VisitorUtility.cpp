...
void VisitorUtility::addAllVariableRelationshipFrom(
        std::shared_ptr<ASTNode> root, Statement s, std::vector<std::shared_ptr<ASTNode>> parents,
    ...
    while (!frontier.empty()) {
        ...
        if (ptr) {
            ...
            funcStmt(std::make_shared<Statement>(s), std::make_shared<Variable>(ptr->getVarName()));
            ...
            ProcedureNode *procedurePtr = dynamic_cast<ProcedureNode *>(proc.get());
            assert(procedurePtr != nullptr);
            funcProc(std::make_shared<Procedure>(procedurePtr->getProcedureName()),
                     std::make_shared<Variable>(ptr->getVarName()));
        }
