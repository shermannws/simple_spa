...
/**
 * @class ResultHandler class generates a result table from the combining two
 * tables and handles result table join operations.
 */
class ResultHandler {
private:
    /**
     ...
     */
    std::shared_ptr<Result> join(std::shared_ptr<Result> r1,
     ...
     */
    std::unordered_map<int, int> getCommonColumns(std::shared_ptr<Result> r1,
                                                  std::shared_ptr<Result> r2);
     ...
     */
    SynonymMap buildSynIndices(std::shared_ptr<Result> r1,
                               std::shared_ptr<Result> r2);
    ...
    /**
     * Returns true if the two rows have the same value for the common synonyms
     * (if any)
     * @param row1 row from table 1
     * @param row2 row from table 2
     * @return boolean
     */
    bool isMatch(const std::vector<Entity> &row1,
...
public:
     ...
     */
    std::shared_ptr<Result> getCombined(std::shared_ptr<Result> r1,
                                        std::shared_ptr<Result> r2);

    /**
     * Casts a Result object into a False Boolean Result if it is an Empty Tuple
     * Result, otherwise returns the original result
