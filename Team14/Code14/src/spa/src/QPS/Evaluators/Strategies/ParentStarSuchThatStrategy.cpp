...
ParentStarSuchThatStrategy::ParentStarSuchThatStrategy(
        std::shared_ptr<PkbReader> pkbReader)
    : SuchThatStrategy(std::move(pkbReader)) {}

std::shared_ptr<Result>
ParentStarSuchThatStrategy::evaluateSynSyn(Ref &leftRef, Ref &rightRef) const {
    ...
    res->setTuples(pkbReader->getParentStarPair(
...
std::shared_ptr<Result>
ParentStarSuchThatStrategy::evaluateSynAny(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto leftEntityType = leftRef.getEntityType();
    auto leftSyn = leftRef.getRep();
    if (rightRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getParentStarTypeStmt(
                QPSUtil::entityToStmtMap.at(leftEntityType), s));
    } else {
        res->setTuples(pkbReader->getParentStarTypeWildcard(
                QPSUtil::entityToStmtMap.at(leftEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
ParentStarSuchThatStrategy::evaluateAnySyn(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto rightEntityType = rightRef.getEntityType();
    auto rightSyn = rightRef.getRep();
    if (leftRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getParentStarStmtType(
                s, QPSUtil::entityToStmtMap.at(rightEntityType)));
    } else {
        res->setTuples(pkbReader->getParentStarWildcardType(
                QPSUtil::entityToStmtMap.at(rightEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
ParentStarSuchThatStrategy::evaluateBoolean(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
