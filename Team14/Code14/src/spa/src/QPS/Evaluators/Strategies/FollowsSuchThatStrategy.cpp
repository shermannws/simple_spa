...
FollowsSuchThatStrategy::FollowsSuchThatStrategy(
        std::shared_ptr<PkbReader> pkbReader)
    : SuchThatStrategy(std::move(pkbReader)) {}

std::shared_ptr<Result>
FollowsSuchThatStrategy::evaluateSynSyn(Ref &leftRef, Ref &rightRef) const {
    ...
    res->setTuples(pkbReader->getFollowsPair(
...
std::shared_ptr<Result>
FollowsSuchThatStrategy::evaluateSynAny(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto leftEntityType = leftRef.getEntityType();
    auto leftSyn = leftRef.getRep();
    if (rightRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getFollowsTypeStmt(
                QPSUtil::entityToStmtMap.at(leftEntityType), s));
    } else {
        res->setTuples(pkbReader->getFollowsTypeWildcard(
                QPSUtil::entityToStmtMap.at(leftEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
FollowsSuchThatStrategy::evaluateAnySyn(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto rightEntityType = rightRef.getEntityType();
    auto rightSyn = rightRef.getRep();
    if (leftRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getFollowsStmtType(
                s, QPSUtil::entityToStmtMap.at(rightEntityType)));
    } else {
        res->setTuples(pkbReader->getFollowsWildcardType(
                QPSUtil::entityToStmtMap.at(rightEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
FollowsSuchThatStrategy::evaluateBoolean(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
