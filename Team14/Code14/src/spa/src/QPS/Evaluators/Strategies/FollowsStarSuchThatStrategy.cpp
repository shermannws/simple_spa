...
FollowsStarSuchThatStrategy::FollowsStarSuchThatStrategy(
        std::shared_ptr<PkbReader> pkbReader)
    : SuchThatStrategy(std::move(pkbReader)) {}

std::shared_ptr<Result>
FollowsStarSuchThatStrategy::evaluateSynSyn(Ref &leftRef, Ref &rightRef) const {
    ...
    res->setTuples(pkbReader->getFollowsStarPair(
...
std::shared_ptr<Result>
FollowsStarSuchThatStrategy::evaluateSynAny(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto leftEntityType = leftRef.getEntityType();
    auto leftSyn = leftRef.getRep();
    if (rightRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getFollowsStarTypeStmt(
                QPSUtil::entityToStmtMap.at(leftEntityType), s));
    } else {
        res->setTuples(pkbReader->getFollowsStarTypeWildcard(
                QPSUtil::entityToStmtMap.at(leftEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
FollowsStarSuchThatStrategy::evaluateAnySyn(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto rightEntityType = rightRef.getEntityType();
    auto rightSyn = rightRef.getRep();
    if (leftRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getFollowsStarStmtType(
                s, QPSUtil::entityToStmtMap.at(rightEntityType)));
    } else {
        res->setTuples(pkbReader->getFollowsStarWildcardType(
                QPSUtil::entityToStmtMap.at(rightEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
FollowsStarSuchThatStrategy::evaluateBoolean(Ref &leftRef,
                                             Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
