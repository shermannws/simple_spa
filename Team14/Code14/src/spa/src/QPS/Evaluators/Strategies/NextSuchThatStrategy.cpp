...
NextSuchThatStrategy::evaluateSynSyn(Ref &leftRef, Ref &rightRef) const {
    ...
    res->setTuples(pkbReader->getNextPair(
...
std::shared_ptr<Result>
NextSuchThatStrategy::evaluateSynAny(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto leftEntityType = leftRef.getEntityType();
    if (rightRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getNextTypeStmt(
                QPSUtil::entityToStmtMap.at(leftEntityType), s));
    } else {
        res->setTuples(pkbReader->getNextTypeWildcard(
                QPSUtil::entityToStmtMap.at(leftEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
NextSuchThatStrategy::evaluateBoolean(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
