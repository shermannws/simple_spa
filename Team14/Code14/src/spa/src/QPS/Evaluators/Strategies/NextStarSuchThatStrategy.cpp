...
std::shared_ptr<Result>
NextStarSuchThatStrategy::evaluateSynAny(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
    auto leftEntityType = leftRef.getEntityType();
    if (rightRef.isRootType(RootType::Integer)) {
        ...
        res->setTuples(pkbReader->getNextStarTypeStmt(
                QPSUtil::entityToStmtMap.at(leftEntityType), s));
    } else {
        res->setTuples(pkbReader->getNextStarTypeWildcard(
                QPSUtil::entityToStmtMap.at(leftEntityType)));
    }
    return res;
}
...
std::shared_ptr<Result>
NextStarSuchThatStrategy::evaluateBoolean(Ref &leftRef, Ref &rightRef) const {
    std::shared_ptr<Result> res = std::make_shared<Result>();
