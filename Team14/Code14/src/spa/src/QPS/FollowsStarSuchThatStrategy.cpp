...
Result FollowsStarSuchThatStrategy::evaluateClause(Clause& clause, std::shared_ptr<PkbReader> pkbReader) const {
    ...
    if (leftRootType == RootType::Synonym && rightRootType == RootType::Synonym) { // Follows*(s1,s2)
        ...
        tuples = pkbReader->getFollowsStarPair(stmtMap.at(leftEntityType), stmtMap.at(rightEntityType));
        ...
        auto data = pkbReader->getFollowsStarTypeStmt(stmtMap.at(leftEntityType), s);
        ...
        std::string syn = leftRef.getRep();
        auto data = pkbReader->getFollowsStarTypeWildcard(stmtMap.at(leftEntityType));
        for (const auto& ent : data) {
            std::vector<Entity> tuple_vector {ent};
            tuples.emplace_back(tuple_vector);
        }
        ...
        auto data = pkbReader->getFollowsStarStmtType(s, stmtMap.at(rightEntityType));
        for (const auto& ent : data) {
            std::vector<Entity> tuple_vector {ent};
            tuples.emplace_back(tuple_vector);
        }
