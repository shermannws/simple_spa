...
void EntrefExprSpecHandler::handle(const Query &query,
    ...
    if (leftRef.getRootType() == RootType::Synonym &&
        leftRef.getEntityType() != QueryEntityType::Variable) {
        throw SemanticException(
                "Invalid LHS in Pattern, synonym does not represent variable");
    }
