...
void ProcProcHandler::handleRefType(Ref &leftRef, Ref &rightRef) {
    ...
    if (leftRef.getRootType() == RootType::Synonym && leftRef.getEntityType() != QueryEntityType::Procedure) {
        throw SemanticException("Invalid LHS, non-procedure synonym");
    }

    if (rightRef.getRootType() == RootType::Synonym && rightRef.getEntityType() != QueryEntityType::Procedure) {
