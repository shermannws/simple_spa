...
ExprSpecParser::ExprSpecParser(std::shared_ptr<Tokenizer> tokenizer)
    : tokenizer(tokenizer) {}
        ...
        if (!curr->isToken(TokenType::Quote)) {// WILDCARD
            ...
            if (!curr->isToken(TokenType::Underscore)) {
                throw SyntaxException(
                        "Invalid Expression Spec, missing trailing _");
            }
...
Expression ExprSpecParser::extractExpression() {
    ...
    while (!curr->isToken(TokenType::Quote)) {
        ...
        if (curr->isOperand()) {
            ...
            if (operators.empty() ||
                !operators.top()->isToken(
                        TokenType::Lparenthesis)) {// pop Lparentheses
                throw SyntaxException("Invalid expression spec");
            }
            operators.pop();
    ...
    if (expression.empty() ||
        expression.size() >
                1) {// empty expression OR too many factors e.g "x y"
        throw SyntaxException("Invalid Expression Spec");
    }
...
void ExprSpecParser::processSubExpr(
        std::stack<std::shared_ptr<Token>> &operators,
        std::stack<std::string> &expression) {
...
bool ExprSpecParser::validateExprSyntax(
        std::vector<std::shared_ptr<Token>> &input) {
    size_t currIndex = 0;
    return isExpr(input, currIndex) && currIndex == input.size();
}

bool ExprSpecParser::isExpr(std::vector<std::shared_ptr<Token>> &input,
                            size_t &index) {
...
bool ExprSpecParser::isTerm(std::vector<std::shared_ptr<Token>> &input,
                            size_t &index) {
    if (index >= input.size()) { return false; }
...
bool ExprSpecParser::isFactor(std::vector<std::shared_ptr<Token>> &input,
                              size_t &index) {
    if (index >= input.size()) { return false; }
    ...
    if (input[index]->isToken(TokenType::Lparenthesis)) {
        ...
        if (hasValidExpr && index < input.size()) {
            bool hasRParenthesis =
                    input[index]->isToken(TokenType::Rparenthesis);
            index++;
