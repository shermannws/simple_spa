...
std::vector<Entity>
Assignment::getStmtVarPairFromAssign(const Assignment &assignment) {
...
std::size_t
std::hash<Assignment>::operator()(const Assignment &assignment) const {
    return std::hash<Entity>()(*assignment.getStatement());
}

std::size_t std::hash<std::shared_ptr<Assignment>>::operator()(
        const std::shared_ptr<Assignment> assignmentPtr) const {
