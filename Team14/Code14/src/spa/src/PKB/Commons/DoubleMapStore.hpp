...
template<typename K1, typename K2, typename V1, typename V2, typename H1,
         typename H2>
void DoubleMapStore<K1, K2, V1, V2, H1, H2>::store(std::shared_ptr<K1> left,
                                                   std::shared_ptr<K2> right) {
...
template<typename K1, typename K2, typename V1, typename V2, typename H1,
         typename H2>
std::shared_ptr<V1> DoubleMapStore<K1, K2, V1, V2, H1, H2>::getRightEntitiesOf(
        std::shared_ptr<K1> left) const {
    if (this->leftToRightStore.find(left) == this->leftToRightStore.end()) {
        return nullptr;
    }
...
template<typename K1, typename K2, typename V1, typename V2, typename H1,
         typename H2>
std::shared_ptr<V2> DoubleMapStore<K1, K2, V1, V2, H1, H2>::getLeftEntitiesOf(
        std::shared_ptr<K2> right) const {
    if (this->rightToLeftStore.find(right) == this->rightToLeftStore.end()) {
        return nullptr;
    }
...
typename std::unordered_map<std::shared_ptr<K1>,
                            typename std::shared_ptr<V1>>::iterator
...
typename std::unordered_map<std::shared_ptr<K2>,
                            typename std::shared_ptr<V2>>::iterator
