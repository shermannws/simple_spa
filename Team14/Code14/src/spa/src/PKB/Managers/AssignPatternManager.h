...
/**
 * The AssignPatternManager class is used to store and retrieve Assignment
 * entities from the PKB. It is also used to query for Assignment entities that
...
private:
    ...
    /**
     * Matches the expression with the pattern using regex. Returns true if the
     * Expression matches the pattern, else false. Uses regex_match hasWildCard
     * is false look for exact match. Uses regex_search if hasWildCard is true
     * to look for partial match.
     ...
     */
    bool matchExpression(Expression &expression, std::regex &pattern,
                         bool hasWildCard) const;

    /**
     * Parses the pattern to a regex object. Handles the escaping of special
     * characters.
