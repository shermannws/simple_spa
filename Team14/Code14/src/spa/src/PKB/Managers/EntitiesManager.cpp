...
template<typename E>
std::vector<Entity>
EntitiesManager::getEntities(std::shared_ptr<EntityStore<E>> store,
                             std::function<bool(E &)> matcher) const {
    return ManagerUtils::getEntitiesFromEntityStore<E>(
            store, matcher, [](Entity &entity) { return entity; });
}

template<typename E>
std::vector<Entity>
EntitiesManager::getAllEntities(std::shared_ptr<EntityStore<E>> store) const {
...
std::vector<Entity> EntitiesManager::getAllRead() const {
    return this->getEntities<Statement>(
            this->statementStore, [](Statement &statement) {
...
EntitiesManager::getStatement(std::shared_ptr<Statement> s) const {
    return this->statementStore->getEntity(s);
}

std::shared_ptr<Entity>
EntitiesManager::getVariable(std::shared_ptr<Variable> v) const {
    return this->variableStore->getEntity(v);
}

std::shared_ptr<Entity>
EntitiesManager::getConstant(std::shared_ptr<Constant> c) const {
    return this->constantStore->getEntity(c);
}

std::shared_ptr<Entity>
EntitiesManager::getProcedure(std::shared_ptr<Procedure> p) const {
