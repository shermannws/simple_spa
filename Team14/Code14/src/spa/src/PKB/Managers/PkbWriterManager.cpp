...
PkbWriterManager::PkbWriterManager(
    ...
    : assignmentManager(assignmentManager), entitiesManager(entitiesManager),
      ...
      nextRelationshipManager(nextRelationshipManager), cfgManager(cfgManager) {
}
...
void PkbWriterManager::addAssignPattern(std::shared_ptr<Statement> s,
                                        std::shared_ptr<Variable> lhs,
                                        std::shared_ptr<Expression> rhs) {
    std::shared_ptr<Assignment> assignStmt =
            std::make_shared<Assignment>(s, lhs, rhs);
    this->assignmentManager->storeAssignPattern(assignStmt);
}

void PkbWriterManager::addFollowsRelationship(std::shared_ptr<Statement> s1,
                                              std::shared_ptr<Statement> s2,
                                              bool isDirect) {
    this->followsRelationshipManager->storeRelationship(s1, s2, isDirect);
}

void PkbWriterManager::addUsesRelationship(std::shared_ptr<Statement> s,
                                           std::shared_ptr<Variable> v) {
    this->usesRelationshipManager->storeRelationship(s, v);
}

void PkbWriterManager::addModifiesRelationship(std::shared_ptr<Statement> s,
                                               std::shared_ptr<Variable> v) {
    this->modifiesRelationshipManager->storeRelationship(s, v);
}

void PkbWriterManager::addParentRelationship(std::shared_ptr<Statement> s1,
                                             std::shared_ptr<Statement> s2,
                                             bool isDirect) {
    this->parentRelationshipManager->storeRelationship(s1, s2, isDirect);
}

void PkbWriterManager::addCallsRelationship(std::shared_ptr<Procedure> p1,
                                            std::shared_ptr<Procedure> p2) {
    this->callsRelationshipManager->storeRelationship(p1, p2, true);
}

void PkbWriterManager::addModifiesProcRelationship(
        std::shared_ptr<Procedure> p, std::shared_ptr<Variable> v) {
    this->modifiesProcRelationshipManager->storeRelationship(p, v);
}

void PkbWriterManager::addUsesProcRelationship(std::shared_ptr<Procedure> p,
                                               std::shared_ptr<Variable> v) {
...
void PkbWriterManager::addProcedureToStatementsMap(
...
void PkbWriterManager::triggerProcToVarTransitiveCalculation() {
    this->modifiesProcRelationshipManager
            ->calculateProcVarRelationshipForCallers(
                    this->callsRelationshipManager);
    this->usesProcRelationshipManager->calculateProcVarRelationshipForCallers(
            this->callsRelationshipManager);
}
...
void PkbWriterManager::triggerStmtToVarTransitiveCalculation() {
    ManagerUtils::addStmtVarFromProcVar(
            ...
            std::make_shared<RelationshipStore<Procedure, Statement>>(
                    this->tempProcedureToStatementsMap),
            this->modifiesProcRelationshipManager);
    ManagerUtils::addStmtVarFromProcVar(
...
void PkbWriterManager::triggerTransitiveCalc() {
    // The order of these 3 calls are important, as each transitivity
    // calculation depends on the previous one
    triggerCallsTransitiveCalculation();
    triggerProcToVarTransitiveCalculation();
    triggerStmtToVarTransitiveCalculation();
}

void PkbWriterManager::addNextRelationship(std::shared_ptr<Statement> s1,
                                           std::shared_ptr<Statement> s2) {
    this->nextRelationshipManager->storeRelationship(s1, s2, true);
}

void PkbWriterManager::setCFGMap(
