...
void PkbConcreteWriter::addAssignStatement(std::shared_ptr<Statement> s) {
    this->writerManager->addStatement(s);
}

void PkbConcreteWriter::addAssignPattern(std::shared_ptr<Statement> s,
                                         std::shared_ptr<Variable> lhs,
                                         std::shared_ptr<Expression> rhs) {
    this->writerManager->addAssignPattern(s, lhs, rhs);
}

void PkbConcreteWriter::addFollowsRelationship(std::shared_ptr<Statement> s1,
                                               std::shared_ptr<Statement> s2,
                                               bool isDirect) {
    this->writerManager->addFollowsRelationship(s1, s2, isDirect);
}

void PkbConcreteWriter::addUsesRelationship(std::shared_ptr<Statement> s,
...
void PkbConcreteWriter::addModifiesRelationship(std::shared_ptr<Statement> s,
                                                std::shared_ptr<Variable> v) {
    this->writerManager->addModifiesRelationship(s, v);
}

void PkbConcreteWriter::addParentRelationship(std::shared_ptr<Statement> s1,
                                              std::shared_ptr<Statement> s2,
                                              bool isDirect) {
    this->writerManager->addParentRelationship(s1, s2, isDirect);
}

void PkbConcreteWriter::addCallsRelationship(std::shared_ptr<Procedure> p1,
                                             std::shared_ptr<Procedure> p2) {
    this->writerManager->addCallsRelationship(p1, p2);
}

void PkbConcreteWriter::addProcedureToStatementsMap(
...
void PkbConcreteWriter::addModifiesProcRelationship(
        std::shared_ptr<Procedure> p, std::shared_ptr<Variable> v) {
    this->writerManager->addModifiesProcRelationship(p, v);
}

void PkbConcreteWriter::addUsesProcRelationship(std::shared_ptr<Procedure> p,
                                                std::shared_ptr<Variable> v) {
    this->writerManager->addUsesProcRelationship(p, v);
}

void PkbConcreteWriter::addNextRelationship(std::shared_ptr<Statement> s1,
                                            std::shared_ptr<Statement> s2) {
    this->writerManager->addNextRelationship(s1, s2);
}

void PkbConcreteWriter::setCFGMap(
